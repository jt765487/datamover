...
def make_fs_stub():
    ...
    def resolve_side_effect(p, strict=False):
        ...
        if strict and not fs.exists(path_obj): # fs.exists is also mockable per test
             raise FileNotFoundError(f"Mock FileNotFoundError for {path_obj} (strict=True)")
        return path_obj
...
def ensure_logger_dir_default(tmp_path):
    ...
    if not default_logger_path.exists(): # Only create if it doesn't exist on test runner
...
def test_load_config_happy_path(config_file):
    ...
    fs.exists.side_effect = lambda p: p == Path("/tmp/logs").expanduser() or p == Path("/tmp/base").expanduser()
...
def test_missing_option_raises(section, key, tmp_path, config_file): # Added config_file to make replacement easier
    ...
    fs = make_fs_stub()
    fs.exists.side_effect = lambda p: p == Path("/tmp/logs").expanduser() or p == Path("/tmp/base").expanduser() or p == cfg_path
...
def test_logger_dir_checks(config_file, tmp_path): # tmp_path is used by config_file
    ...
    fs.exists.side_effect = lambda p: p == Path("/tmp/base").expanduser() or p == config_file

    # Scenario 1: logger_dir does not exist
    mock_logger_path = Path("/tmp/non_existent_logs").expanduser()
    modified_ini = config_file.read_text().replace("logger_dir = /tmp/logs", f"logger_dir = {mock_logger_path}")
    cfg_path_scenario1 = tmp_path / "s1_config.ini"
    cfg_path_scenario1.write_text(modified_ini)

    fs_s1 = make_fs_stub()
    fs_s1.exists.side_effect = lambda p: p == Path("/tmp/base").expanduser() or \
                                       p == cfg_path_scenario1 or \
                                       (p == mock_logger_path and False) # logger_dir doesn't exist
    ...
    modified_ini_s2 = config_file.read_text().replace("logger_dir = /tmp/logs", f"logger_dir = {mock_logger_file_path}")
    cfg_path_scenario2 = tmp_path / "s2_config.ini"
    cfg_path_scenario2.write_text(modified_ini_s2)

    fs_s2 = make_fs_stub()
    fs_s2.exists.side_effect = lambda p: p == Path("/tmp/base").expanduser() or \
                                        p == cfg_path_scenario2 or \
                                        p == mock_logger_file_path # logger_path "exists"
    fs_s2.is_dir.side_effect = lambda p: p != mock_logger_file_path # logger_path is NOT a dir
    fs_s2.is_file.side_effect = lambda p: p == cfg_path_scenario2 or \
                                         p == mock_logger_file_path # config is a file, logger path is a file
...
def test_scanner_timeouts_validation(tmp_path, config_file):
    txt = config_file.read_text().replace("lost_timeout_seconds = 3.0", "lost_timeout_seconds = 5.0") # 5.0 > 4.0
    cfg_path = tmp_path / "config_modified.ini"
    cfg_path.write_text(txt)
...
def test_uploader_backoff_validation(tmp_path, config_file):
    ...
    for line in lines:
        if line.strip().startswith("initial_backoff ="):
            ...
            output_lines.append("max_backoff = 1.0")  # This makes initial (2.0) > max (1.0)
        else:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("pcap_extension_no_dot", ".pcap", "pcap_extension_no_dot must be a non-empty string without a dot"),
        ("csv_extension_no_dot", "", f"'{'csv_extension_no_dot'}' cannot be empty"),
        ("csv_extension_no_dot", ".csv", "csv_extension_no_dot must be a non-empty string without a dot"),
    ],
)
def test_bad_file_extensions(tmp_path, key_to_modify, bad_value, expected_error_msg_part, config_file):
    original_content = config_file.read_text()
    ...
    fs.exists.side_effect = lambda p: p in (Path("/tmp/logs").expanduser(), Path("/tmp/base").expanduser(), cfg_path)
...
def test_numeric_value_below_min(tmp_path, section, key, bad_value, min_value_text_for_msg, config_file):
    original_content = config_file.read_text()
    lines = original_content.splitlines()
    modified_lines = []
    found = False
...
def test_empty_remote_host_url(tmp_path, config_file):
    modified_ini = config_file.read_text().replace("remote_host_url = http://example.com", "remote_host_url = ")
    cfg_path = tmp_path / "config_modified.ini"
    cfg_path.write_text(modified_ini)
    fs = make_fs_stub()
    fs.exists.side_effect = lambda p: p in (Path("/tmp/logs").expanduser(), Path("/tmp/base").expanduser(), cfg_path)
...
def test_load_config_unicode_decode_error(tmp_path):
    ...
    config_path.write_bytes(b"\xff\xfe[Directories]\nbase_dir = /tmp/base\n") # Invalid UTF-8 start
    fs = make_fs_stub()
...
def test_load_config_malformed_ini(tmp_path, malformed_ini_content, expected_exception_type):
    cfg_path = tmp_path / "config.ini"
...
def test_logger_dir_resolve_os_error(tmp_path, config_file):
    ...
    original_resolve = fs.resolve.side_effect
    def custom_resolve(p, strict=False):
        expanded_p = Path(p).expanduser()
        if expanded_p == default_logger_path and strict: # Error only for logger_dir's strict resolve
...
def test_base_dir_resolve_unexpected_error(config_file):
    ...
    def custom_resolve_base(p, strict=False):
        expanded_p = Path(p).expanduser()
        if expanded_p == Path("/tmp/base").expanduser() and not strict: # Error for base_dir (non-strict)
...
def test_unexpected_error_during_section_parsing(config_file):
    fs = make_fs_stub()
    fs.exists.side_effect = lambda p: p in (Path("/tmp/logs").expanduser(), Path("/tmp/base").expanduser(), config_file)


    with patch("datamover.startup_code.load_config._parse_files_section_config", side_effect=ValueError("Internal parse boom")):
        with pytest.raises(ConfigError) as exc:
            load_config(str(config_file), fs=fs)
