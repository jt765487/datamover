...
# -----------------------------------------------------------------------------
VERSION="2.3.3" # Ensure -v is passed to all sub-scripts consistently
...
General Options:
  ...
  -c, --config FILE         Base install config filename (inside source-dir).
                            Default: $BASE_CONFIG
  -i, --instances "LIST"    Comma-separated list of instance names.
                            Example: -i inst1,inst2,inst3
                            or if using spaces: -i "inst1, inst2, inst3"
  ...
  -v, --verbose             Enable verbose shell tracing (set -x) and debug messages for
                            orchestrator and sub-scripts.
...
main() {
  ...
  if [[ "$OPERATION_MODE" == "status-only" ]]; then
    ...
    # Options for manage_services.sh in status-only mode
    local manage_status_opts=()
    if [[ "$DRY_RUN" == true ]]; then manage_status_opts+=("-n"); fi
    if [[ "$VERBOSE_MODE" == true ]]; then manage_status_opts+=("-v"); fi

    info "--- Bitmover Service Status ---"
    run ./manage_services.sh "${manage_status_opts[@]}" --status
    ...
    if (( ${#PARSED_INSTANCE_NAMES[@]} > 0 )); then
      ...
      for inst_name_status_check in "${PARSED_INSTANCE_NAMES[@]}"; do
        run ./manage_services.sh "${manage_status_opts[@]}" -i "$inst_name_status_check" --status
      done
  ...
  if [[ "$VERBOSE_MODE" == true ]]; then base_install_cmd_array_main+=("-v"); fi # Pass -v
  ...
  if [[ "$OPERATION_MODE" == "install" ]]; then
    if (( ${#PARSED_INSTANCE_NAMES[@]} > 0 )); then
      ...
      # Note: -v for configure_instance.sh will be added inside the loop
      ...
      for inst_name_config_main in "${PARSED_INSTANCE_NAMES[@]}"; do
        ...
        if [[ "$VERBOSE_MODE" == true ]]; then current_configure_cmd_array_main+=("-v"); fi # Pass -v
  ...
  # Define base options for manage_services.sh once
  local manage_service_base_opts_array_main=()
  if [[ "$DRY_RUN" == true ]]; then manage_service_base_opts_array_main+=("-n"); fi
  if [[ "$VERBOSE_MODE" == true ]]; then manage_service_base_opts_array_main+=("-v"); fi # Pass -v to manage_services.sh
